...
# ... (st.table(ms_df)로 사주 명식표 표시, st.caption(saju_year_caption) 실행된 후) ...

            # --- [시작] 일간 기준 12운성 (일간포태) 표시 및 해설 ---
            if locals().get('analysis_possible', False) and \
               locals().get('day_gan_char') and locals().get('day_gan_char') not in ["?", "오류", "입력오류", "계산불가"]:

                st.markdown("---") # 구분선
                st.subheader(f"📖 일간({day_gan_char}) 기준 12운성 (일간포태) 및 해설")
                
                # 선택적 디버깅: 각 지지 값을 확인하려면 아래 주석을 해제하세요.
                # st.text(f"DEBUG (일간포태 입력값): DayGan='{day_gan_char}', YearJi='{year_ji_char}', MonthJi='{month_ji_char}', DayJi='{day_ji_char}', TimeJi='{time_ji_char}'")

                ilgan_potae_display_data = []  # 화면 테이블 표시용 데이터
                ilgan_potae_guideline_texts = [] # 클립보드 복사 내용에 추가될 요약 텍스트
                ilgan_potae_segment_texts_detail = [] # "전체 풀이 내용"에 추가될 상세 텍스트

                # 각 지지 변수가 존재하는지 확인하고 사용
                available_jijis = {}
                if 'year_ji_char' in locals() and year_ji_char and year_ji_char not in ["?", "오류"]:
                    available_jijis["년지"] = year_ji_char
                if 'month_ji_char' in locals() and month_ji_char and month_ji_char not in ["?", "오류"]:
                    available_jijis["월지"] = month_ji_char
                if 'day_ji_char' in locals() and day_ji_char and day_ji_char not in ["?", "오류"]:
                    available_jijis["일지"] = day_ji_char
                if 'time_ji_char' in locals() and time_ji_char and time_ji_char not in ["?", "오류"]:
                    available_jijis["시지"] = time_ji_char
                
                potae_order = ["년지", "월지", "일지", "시지"] # 표시 순서

                for ji_label in potae_order:
                    if ji_label in available_jijis:
                        current_ji_char = available_jijis[ji_label]
                        
                        # 이전에 "신사일주 월지 해수" 문제 디버깅을 위해, 필요시 아래 주석 해제
                        # if ji_label == "월지":
                        #     st.info(f"DEBUG (일간포태-월지 계산 전): day_gan_char='{day_gan_char}', current_month_ji_char='{current_ji_char}'")
                        
                        unseong_val = get_12_unseong(day_gan_char, current_ji_char)
                        
                        if unseong_val and unseong_val not in ["?", "계산불가", "입력오류"]:
                            explanation = get_12_unseong_explanation(unseong_val)
                            ilgan_potae_display_data.append({
                                "기준": f"일간 vs {ji_label}({current_ji_char})",
                                "12운성": unseong_val,
                                "간략 해설": explanation
                            })
                            # 클립보드용은 운성 이름만 간결하게
                            ilgan_potae_guideline_texts.append(f"vs {ji_label}({current_ji_char}): {unseong_val}")
                            # 전체 풀이 내용용은 해설 첫 문장 포함
                            ilgan_potae_segment_texts_detail.append(f"vs {ji_label}({current_ji_char}): **{unseong_val}** ({explanation.split('.')[0]}.)")
                        else:
                            # 12운성 계산 불가 또는 유효하지 않은 값인 경우
                            ilgan_potae_display_data.append({
                                "기준": f"일간 vs {ji_label}({current_ji_char})", "12운성": unseong_val, "간략 해설": "정보 없음"
                            })
                            ilgan_potae_guideline_texts.append(f"vs {ji_label}({current_ji_char}): {unseong_val if unseong_val else '정보없음'}")
                            ilgan_potae_segment_texts_detail.append(f"vs {ji_label}({current_ji_char}): {unseong_val if unseong_val else '정보없음'} (해설 없음)")


                if ilgan_potae_display_data:
                    ilgan_potae_df = pd.DataFrame(ilgan_potae_display_data)
                    if not ilgan_potae_df.empty:
                        # 컬럼 순서 지정 (선택 사항)
                        if {"기준", "12운성", "간략 해설"}.issubset(ilgan_potae_df.columns):
                             ilgan_potae_df = ilgan_potae_df[["기준", "12운성", "간략 해설"]]
                        st.table(ilgan_potae_df.set_index("기준")) # Streamlit 앱 화면에 테이블 형태로 표시
                    else:
                        st.text("일간 기준 12운성 정보를 표시할 데이터가 없습니다.")

                    # "전체 풀이 내용 다시 보기" (st.session_state.interpretation_segments)에 추가
                    if ilgan_potae_segment_texts_detail:
                        potae_full_text_for_segment = f"**일간({day_gan_char}) 기준 12운성 (일간포태) 및 간략 해설:**\n" + "\n".join([f"- {s}" for s in ilgan_potae_segment_texts_detail])
                        st.session_state.interpretation_segments.append(("📖 일간 기준 12운성 (일간포태)", potae_full_text_for_segment))

                    # 클립보드 복사 내용 (guideline_parts)에 요약본 추가
                    if ilgan_potae_guideline_texts:
                        guideline_parts.append(f"일간({day_gan_char}) 기준 12운성 (일간포태) ▶ {', '.join(ilgan_potae_guideline_texts)}")
                else:
                    no_potae_info_text_ui = "일간 기준 12운성 정보를 계산할 수 있는 유효한 지지 정보가 없거나 일간 정보가 명확하지 않습니다."
                    st.text(no_potae_info_text_ui)
                    st.session_state.interpretation_segments.append(("📖 일간 기준 12운성 (일간포태)", no_potae_info_text_ui))
                    if locals().get('day_gan_char') and day_gan_char not in ["?", "오류", "입력오류", "계산불가"]:
                         guideline_parts.append(f"일간({day_gan_char}) 기준 12운성 (일간포태) ▶ 정보 없음")
            else:
                # analysis_possible이 False이거나 day_gan_char가 유효하지 않은 경우
                st.markdown("---") # 구분선
                st.text("일간 정보가 유효하지 않아 '일간 기준 12운성(일간포태)'을 표시할 수 없습니다.")
            # --- [끝] 일간 기준 12운성 (일간포태) 표시 및 해설 ---
        ...
        # 그리고 saju_year_val 변수가 모두 올바르게 계산되었다고 가정합니다.
